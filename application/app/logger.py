...
HTTP_LOGGING = (HTTP_LOGGING == "True" or HTTP_LOGGING ==
                "true" or HTTP_LOGGING == "1" or HTTP_LOGGING)
...
class CustomLogHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        response_code = post_with_retries(
            self.url, log_entry, retries=3, backoff=1)
        print(response_code)
        return
...
class ApplicationLogger():
    def __init__(
        ...
        customHandler = CustomLogHandler(
            url=f"{LOGGING_URL}/{logging_endpoint}")
